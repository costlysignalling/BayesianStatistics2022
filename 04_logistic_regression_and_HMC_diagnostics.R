...
#Create the decision variable: weher the body on the right was selected or not
...
m.sex<-ulam(alist(
  ...
  logit(p)<-a[sex],
  a[sex]~dnorm(0,1)
),data=d.sex,cores=4,chains=4,log_lik=T,sample=F)
...
m.vary<-ulam(alist(
  ...
  mean_a~dnorm(0,1),
  sd_rater~dexp(1)
),data=d.rater,cores=4,chains=4,log_lik=T,sample=F)
...
sex<-sample(c("F","M","NB","B"),N,replace=T,prob=c(0.485,0.485,0.02,0.01))
summary(as.factor(sex))

Fh<-170
Mh<-180
mu.height<-c(F=Fh, M=Mh, NB=mean(c(Fh,Mh)), B=Mh*0.9+Fh*0.1) #Average heights per group.
height <- rnorm(N,mu.height[sex],9) # total height of each individual. SD per each group is 9.
...
sex2<-sample(c("F","M","NB","B"),N2,replace=T,prob=c(0.485,0.485,0.02,0.01))
mu.height2<-c(F=Fh, M=Mh, NB=mean(c(Fh,Mh)), B=Mh*0.9+Fh*0.1) #Average heights per group.
height2 <- rnorm(N2,mu.height2[sex2],9) # total height of each individual
